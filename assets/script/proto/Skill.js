//
// Auto Generated Code
//

// Generate From Skill.xlsx
module.exports = {
	1: {
		1: {
			ID: 1,
			Index: 1,
			SkillName: '攻击',
			Target: {"type":"single","team":1,"singing":0.57},
			Actions: {damage:{"dmg":(LvUp) => {return 60+LvUp;}}},
			ActionCount: 0,
			Animation: 'attack_01',
			CriticalTime: 66,
			Effect: 'wsricochet_ice',
			EffectType: [{"type":"bullt","origin":"owner","delay":1.7}],
			EffectiveTime: 151,
			Path: 'chenjinchou',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2: {
		1: {
			ID: 2,
			Index: 1,
			SkillName: '防御',
			Target: {"type":"single","team":0,"singing":0.7},
			Actions: {addBuff:{buffID:2,"time":(LvUp) => {return 15+LvUp;}}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 66,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1001: {
		1: {
			ID: 1001,
			Index: 1,
			SkillName: '穿云指',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":(LvUp) => {return 100+LvUp;}},addBuff:{"buffID":1001,"count":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 2,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'wsricochet_ice',
			EffectType: [{"type":"bullt","origin":"owner","delay":1.7}],
			EffectiveTime: 151,
			Path: 'chenjinchou',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 1001,
			Index: 2,
			SkillName: '穿云指',
			Target: {},
			Actions: {spawnSummoned:{"type":"wSword","num":(LvUp) => {return 1+LvUp;},"area":"random"}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1002: {
		1: {
			ID: 1002,
			Index: 1,
			SkillName: '破云指',
			Target: {"type":"all","team":1,"singing":0.7},
			Actions: {damage:{"dmg":80},addBuff:{"buffID":1001,"count":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 0,
			Animation: 'attack_04',
			CriticalTime: 66,
			Effect: 'frozenrain',
			EffectType: [{"type":"point","origin":"targetCenter"}],
			EffectiveTime: 151,
			Path: 'chenjinchou',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [3.5,3.6,3.7,3.8,4.0,4.3,4.5],
			DmgFlag: ""
		},
		2: {
			ID: 1002,
			Index: 2,
			SkillName: '破云指',
			Target: {},
			Actions: {spawnSummoned:{"type":"wSword","num":(LvUp) => {return 5+LvUp;},"area":"random"}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1003: {
		1: {
			ID: 1003,
			Index: 1,
			SkillName: '敛云招霞',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {addBuff:{"buffID":1003}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1103: {
		1: {
			ID: 1103,
			Index: 1,
			SkillName: '敛云招霞',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'healfog3,healflash',
			EffectType: [{"type":"bullt","origin":"owner"},{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'snow',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1004: {
		1: {
			ID: 1004,
			Index: 1,
			SkillName: '踏月无影',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {addBuff:{"buffID":1004}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'soulofsnow',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1104: {
		1: {
			ID: 1104,
			Index: 1,
			SkillName: '踏月无影',
			Target: {"type":"self","team":0},
			Actions: {create:{"cardID":1001,"rate":0.02,"num":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1005: {
		1: {
			ID: 1005,
			Index: 1,
			SkillName: '密云布雨式',
			Target: {"type":"all","team":1,"singing":0.7},
			Actions: {crushedIce:{"type":"wSword","buffID":1001,"time":(LvUp) => {return 5+LvUp;},"consume":1}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'frag_ice',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 33,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1006: {
		1: {
			ID: 1006,
			Index: 1,
			SkillName: '流云千裂指',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {blackSnow:{buffID:1001,times:(LvUp) => {return 2+LvUp;}}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: '',
			EffectType: [],
			EffectiveTime: 33,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1011: {
		1: {
			ID: 1011,
			Index: 1,
			SkillName: '拨云见日',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {addBuff:{"buffID":1011}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1012: {
		1: {
			ID: 1012,
			Index: 1,
			SkillName: '探囊取物',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {copy:{"num":(LvUp) => {return 1+LvUp;},"pileType":4,"reduceMP":"all","effectTimes":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'shadow_card,hand',
			EffectType: [{"type":"point","origin":"owner"},{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'chuliuxiang_effect',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1013: {
		1: {
			ID: 1013,
			Index: 1,
			SkillName: '碎石觅玉',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {drawCard:{"num":(LvUp) => {return 2+LvUp;},"cardType":2,"piletype":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1014: {
		1: {
			ID: 1014,
			Index: 1,
			SkillName: '窃玉留香',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {addBuff:{"buffID":1014}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1114: {
		1: {
			ID: 1114,
			Index: 1,
			SkillName: '窃玉留香',
			Target: {"type":"single","team":1},
			Actions: {create:{"cardID":1017,"rate":1,"num":(LvUp) => {return 1+LvUp;},"pileType":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1015: {
		1: {
			ID: 1015,
			Index: 1,
			SkillName: '盗帅流芳',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {powerUp:{"powerUp":(LvUp) => {return 0.25+LvUp;},"cardType":1,"pileType":4,"effectTimes":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1016: {
		1: {
			ID: 1016,
			Index: 1,
			SkillName: '抛金引玉',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {dropCard:{"num":(LvUp) => {return 2+LvUp;},"piletype":2,"dropType":"highMP"}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 1016,
			Index: 2,
			SkillName: '抛金引玉',
			Target: {"type":"self","team":0},
			Actions: {normalDrawCard:{"num":(LvUp) => {return 2+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	1017: {
		1: {
			ID: 1017,
			Index: 1,
			SkillName: '郁金香',
			Target: {type:"self","team":0},
			Actions: {addBuff:{"buffID":1017,"time":(LvUp) => {return 0.5+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2001: {
		1: {
			ID: 2001,
			Index: 1,
			SkillName: '女儿红',
			Target: {type:"self","team":0},
			Actions: {addBuff:{"buffID":2001}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2002: {
		1: {
			ID: 2002,
			Index: 1,
			SkillName: '千金沽酒',
			Target: {type:"self","team":0},
			Actions: {drawCard:{"num":2,"cardType":1,"piletype":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2003: {
		1: {
			ID: 2003,
			Index: 1,
			SkillName: '撼树式',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":(LvUp) => {return 30+LvUp;}}},
			ActionCount: 5,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2004: {
		1: {
			ID: 2004,
			Index: 1,
			SkillName: '摧岳式',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":(LvUp) => {return 50+LvUp;}}},
			ActionCount: 4,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 2004,
			Index: 2,
			SkillName: '摧岳式',
			Target: {"type":"single","team":1},
			Actions: {comboTrigger:{"combo":15,"skillID":2104}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2104: {
		1: {
			ID: 2104,
			Index: 1,
			SkillName: '摧岳式',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":(LvUp) => {return 30+LvUp;}}},
			ActionCount: 3,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2005: {
		1: {
			ID: 2005,
			Index: 1,
			SkillName: '裂穹式',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":(LvUp) => {return 120+LvUp;}}},
			ActionCount: 3,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 2005,
			Index: 2,
			SkillName: '裂穹式',
			Target: {type:"self","team":0},
			Actions: {comboTrigger:{"combo":25,"skillID":2105}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2105: {
		1: {
			ID: 2105,
			Index: 1,
			SkillName: '裂穹式',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":(LvUp) => {return 120+LvUp;}}},
			ActionCount: 3,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2006: {
		1: {
			ID: 2006,
			Index: 1,
			SkillName: '乘胜追击',
			Target: {"type":"single","team":1},
			Actions: {comboBoxing:{"dmg":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2011: {
		1: {
			ID: 2011,
			Index: 1,
			SkillName: '以杀为护',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {currentHPDamage:{"dmg":0.1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 2011,
			Index: 2,
			SkillName: '以杀为护',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":2011,"time":15}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2012: {
		1: {
			ID: 2012,
			Index: 1,
			SkillName: '以身相护',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {addBuff:{"buffID":2012,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2013: {
		1: {
			ID: 2013,
			Index: 1,
			SkillName: '易甲换袍',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {exchangeArm:{} },
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2014: {
		1: {
			ID: 2014,
			Index: 1,
			SkillName: '以伤为盾',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":2014}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2114: {
		1: {
			ID: 2114,
			Index: 1,
			SkillName: '以伤为盾',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":2114,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2015: {
		1: {
			ID: 2015,
			Index: 1,
			SkillName: '穿花蝴蝶',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {addBuff:{"buffID":2015}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2016: {
		1: {
			ID: 2016,
			Index: 1,
			SkillName: '狂歌破甲',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":2016}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	2106: {
		1: {
			ID: 2106,
			Index: 1,
			SkillName: '狂歌破甲',
			Target: {"type":"single","team":1},
			Actions: {breakArm:{"breakArm":100}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3001: {
		1: {
			ID: 3001,
			Index: 1,
			SkillName: '兰州巨富',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {addBuff:{"buffID":3001}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 3001,
			Index: 2,
			SkillName: '兰州巨富',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":3008}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3101: {
		1: {
			ID: 3101,
			Index: 1,
			SkillName: '兰州巨富',
			Target: {"type":"all","team":0},
			Actions: {addBuff:{"buffID":3101,"time":300}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3108: {
		1: {
			ID: 3108,
			Index: 1,
			SkillName: '兰州巨富',
			Target: {"type":"all","team":0},
			Actions: {getMP:{"MP":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3002: {
		1: {
			ID: 3002,
			Index: 1,
			SkillName: '寒玉为寄',
			Target: {"type":"single","team":1},
			Actions: {create:{"cardID":3007,"rate":1,"num":1,"pileType":4}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3003: {
		1: {
			ID: 3003,
			Index: 1,
			SkillName: '拂穴疗身',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":3003,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3004: {
		1: {
			ID: 3004,
			Index: 1,
			SkillName: '拂穴韧体',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":3004,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3005: {
		1: {
			ID: 3005,
			Index: 1,
			SkillName: '判官打穴',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":100}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 3005,
			Index: 2,
			SkillName: '判官打穴',
			Target: {"type":"single","team":1},
			Actions: {cleanBuff:{"type":1,"num":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3006: {
		1: {
			ID: 3006,
			Index: 1,
			SkillName: '判官判命',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":50}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 3006,
			Index: 2,
			SkillName: '判官判命',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":3006,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3007: {
		1: {
			ID: 3007,
			Index: 1,
			SkillName: '寒玉',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":3007}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3011: {
		1: {
			ID: 3011,
			Index: 1,
			SkillName: '暗藏酒肉',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":3011}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3012: {
		1: {
			ID: 3012,
			Index: 1,
			SkillName: '精锐化身',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":3012}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3112: {
		1: {
			ID: 3112,
			Index: 1,
			SkillName: '精锐化身',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":3112}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3013: {
		1: {
			ID: 3013,
			Index: 1,
			SkillName: '以命相搏',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {currentHPDamage:{"dmg":0.15}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 3013,
			Index: 2,
			SkillName: '以命相搏',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":150}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3014: {
		1: {
			ID: 3014,
			Index: 1,
			SkillName: '生死作搏',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {currentHPDamage:{"dmg":0.05}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 3014,
			Index: 2,
			SkillName: '生死作搏',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":500}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3015: {
		1: {
			ID: 3015,
			Index: 1,
			SkillName: '蓄命待发',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":3015}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 3015,
			Index: 2,
			SkillName: '蓄命待发',
			Target: {"type":"self","team":0},
			Actions: {damage:{"dmg":800,"delay":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	3016: {
		1: {
			ID: 3016,
			Index: 1,
			SkillName: '问天借命',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":3016,"time":20}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4001: {
		1: {
			ID: 4001,
			Index: 1,
			SkillName: '春风如沐',
			Target: {"type":"single","team":0,"singing":1.3},
			Actions: {heal:{"heal":(LvUp) => {return 200+LvUp;}}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 66,
			Effect: 'healflash,healfog',
			EffectType: [{"type":"point","origin":"target"},{"type":"point","origin":"owner"}],
			EffectiveTime: 434,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4002: {
		1: {
			ID: 4002,
			Index: 1,
			SkillName: '春风送暖',
			Target: {"type":"single","team":0,"singing":1.3},
			Actions: {heal:{"heal":250}},
			ActionCount: 0,
			Animation: 'skill_02',
			CriticalTime: 66,
			Effect: 'healflash,healfog',
			EffectType: [{"type":"point","origin":"target"},{"type":"point","origin":"owner"}],
			EffectiveTime: 434,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 4002,
			Index: 2,
			SkillName: '春风送暖',
			Target: {"type":"self","team":0},
			Actions: {heal:{"heal":250}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4003: {
		1: {
			ID: 4003,
			Index: 1,
			SkillName: '还魂续命',
			Target: {"type":"single","team":0,"dead":1,"singing":1.3},
			Actions: {reliveTarget:{"heal":100}},
			ActionCount: 0,
			Animation: 'skill_02',
			CriticalTime: 66,
			Effect: 'heallight',
			EffectType: [{"type":"point","origin":"target"}],
			EffectiveTime: 434,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4004: {
		1: {
			ID: 4004,
			Index: 1,
			SkillName: '春风化雨',
			Target: {"type":"all","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":4004}},
			ActionCount: 0,
			Animation: 'skill_03',
			CriticalTime: 66,
			Effect: 'healrain',
			EffectType: [{"type":"point","origin":"ownerCenter"}],
			EffectiveTime: 434,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 4004,
			Index: 2,
			SkillName: '春风化雨',
			Target: {"type":"lowHP","team":0},
			Actions: {addBuff:{"buffID":4004}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4005: {
		1: {
			ID: 4005,
			Index: 1,
			SkillName: '兰桂同芳',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":4007,"time":10}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4105: {
		1: {
			ID: 4105,
			Index: 1,
			SkillName: '兰桂同芳',
			Target: {"type":"single","team":0},
			Actions: {reliveTarget:{"ownPercent":0.5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'healfog3,healflash',
			EffectType: [{"type":"bullt","origin":"owner"},{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'snow',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4006: {
		1: {
			ID: 4006,
			Index: 1,
			SkillName: '兰心蕙质',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":4006}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4106: {
		1: {
			ID: 4106,
			Index: 1,
			SkillName: '兰心蕙质',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4011: {
		1: {
			ID: 4011,
			Index: 1,
			SkillName: '五毒轻砂',
			Target: {"type":"single","team":1,"singing":1.3},
			Actions: {damage:{"dmg":50}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'aerolite,healfog',
			EffectType: [{"type":"point","origin":"target"},{"type":"point","origin":"owner"}],
			EffectiveTime: 333,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 4011,
			Index: 2,
			SkillName: '五毒轻砂',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":4011,"time":15}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'poison,wsword',
			EffectType: [{"type":"bounce","frame":8}],
			EffectiveTime: 0,
			Path: 'poison,chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4012: {
		1: {
			ID: 4012,
			Index: 1,
			SkillName: '毒砂印溅',
			Target: {"type":"single","team":1,"singing":1.3},
			Actions: {bounce:{dmg:30,"buffID":4011,"count":1,"time":15,"bounce":2}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'wsricochet_ice',
			EffectType: [{"type":"bounce","frame":8},{"event":"OnBounce","type":"point","origin":"target","effect":"poison","path":"poison"}],
			EffectiveTime: 333,
			Path: 'chenjinchou',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4013: {
		1: {
			ID: 4013,
			Index: 1,
			SkillName: '毒烟弥漫',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":4013}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'poison',
			EffectType: [{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'poison',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4113: {
		1: {
			ID: 4113,
			Index: 1,
			SkillName: '毒烟弥漫',
			Target: {"type":"random","team":1},
			Actions: {addBuff:{"buffID":4011,"time":15}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4014: {
		1: {
			ID: 4014,
			Index: 1,
			SkillName: '弃物取毒',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {drawCard:{"num":2,"cardType":1,"piletype":2}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4015: {
		1: {
			ID: 4015,
			Index: 1,
			SkillName: '炼狱毒火',
			Target: {"type":"random","team":1},
			Actions: {addBuff:{"buffID":4011,"time":15}},
			ActionCount: 9,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	4016: {
		1: {
			ID: 4016,
			Index: 1,
			SkillName: '春风不度',
			Target: {"type":"all","team":1,"singing":1.3},
			Actions: {buffTime:{"buffID":4011,"addTime":10}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'aerolite,healfog',
			EffectType: [{"type":"point","origin":"target"},{"type":"point","origin":"owner"}],
			EffectiveTime: 333,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5001: {
		1: {
			ID: 5001,
			Index: 1,
			SkillName: '烹龙煮凤',
			Target: {"type":"single","team":0},
			Actions: {getMP:{"MP":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5002: {
		1: {
			ID: 5002,
			Index: 1,
			SkillName: '三蛇羹',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":5002,"time":20}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5003: {
		1: {
			ID: 5003,
			Index: 1,
			SkillName: '烤竹鼠',
			Target: {"type":"single","team":0},
			Actions: {randomAddBuff:{"buffIDs":[5003,5007],"rateList":[0.5,0.5],"timeList":[10,10]}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5004: {
		1: {
			ID: 5004,
			Index: 1,
			SkillName: '蒸三臭',
			Target: {"type":"single","team":1},
			Actions: {randomAddBuff:{"buffIDs":[5004,5008],"rateList":[0.5,0.5],"timeList":[10,10]}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5005: {
		1: {
			ID: 5005,
			Index: 1,
			SkillName: '投毒酒中',
			Target: {"type":"single","team":1},
			Actions: {heal:{"heal":(LvUp) => {return 100+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 5005,
			Index: 2,
			SkillName: '投毒酒中',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":5005}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5006: {
		1: {
			ID: 5006,
			Index: 1,
			SkillName: '毒草作烹',
			Target: {"type":"single","team":1},
			Actions: {heal:{"heal":(LvUp) => {return 200+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 5006,
			Index: 2,
			SkillName: '毒草作烹',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":5006}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5011: {
		1: {
			ID: 5011,
			Index: 1,
			SkillName: '游刃有余',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":5011}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5111: {
		1: {
			ID: 5111,
			Index: 1,
			SkillName: '游刃有余',
			Target: {"type":"random","team":1},
			Actions: {create:{"cardID":5018,"rate":1,"num":1,"pileType":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5012: {
		1: {
			ID: 5012,
			Index: 1,
			SkillName: '投掷火雷',
			Target: {"type":"single","team":1},
			Actions: {create:{"cardID":5017,"rate":1,"num":1,"pileType":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5017: {
		1: {
			ID: 5017,
			Index: 1,
			SkillName: '火雷',
			Target: {"type":"self","team":0},
			Actions: {bomb:{"dmg":500,"time":20,"dmgDown":0.2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 5017,
			Index: 2,
			SkillName: '火雷',
			Target: {"type":"self","team":0},
			Actions: {create:{"cardID":5018,"rate":1,"num":2,"pileType":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5018: {
		1: {
			ID: 5018,
			Index: 1,
			SkillName: '小火雷',
			Target: {"type":"self","team":0},
			Actions: {bomb:{"dmg":100,"time":20,"dmgDown":0.2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5013: {
		1: {
			ID: 5013,
			Index: 1,
			SkillName: '高爆火雷',
			Target: {"type":"single","team":1},
			Actions: {create:{"cardID":5019,"rate":1,"num":1,"pileType":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5019: {
		1: {
			ID: 5019,
			Index: 1,
			SkillName: '高爆火雷',
			Target: {"type":"self","team":0},
			Actions: {bomb:{"dmg":800,"time":20,"dmgDown":0.2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5014: {
		1: {
			ID: 5014,
			Index: 1,
			SkillName: '鸣雷',
			Target: {"type":"single","team":1},
			Actions: {create:{"cardID":5020,"rate":1,"num":1,"pileType":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5020: {
		1: {
			ID: 5020,
			Index: 1,
			SkillName: '鸣雷',
			Target: {"type":"self","team":0},
			Actions: {bomb:{"dmg":200,"time":20,"dmgDown":0.2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 5020,
			Index: 2,
			SkillName: '鸣雷',
			Target: {"type":"self","team":0},
			Actions: {breakArm:{"breakArm":300}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		3: {
			ID: 5020,
			Index: 3,
			SkillName: '鸣雷',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":5020,"time":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5015: {
		1: {
			ID: 5015,
			Index: 1,
			SkillName: '雷轰四裂',
			Target: {"type":"single","team":1},
			Actions: {normalDrawCard:{"num":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 5015,
			Index: 2,
			SkillName: '雷轰四裂',
			Target: {"type":"self","team":0},
			Actions: {normalDrawCard:{"num":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	5016: {
		1: {
			ID: 5016,
			Index: 1,
			SkillName: '引火制雷',
			Target: {"type":"self","team":0},
			Actions: {drawCard:{"num":1,"piletype":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6001: {
		1: {
			ID: 6001,
			Index: 1,
			SkillName: '',
			Target: {},
			Actions: {},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6002: {
		1: {
			ID: 6002,
			Index: 1,
			SkillName: '料敌为奇',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":6002,"time":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6003: {
		1: {
			ID: 6003,
			Index: 1,
			SkillName: '料敌为偶',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":6003,"time":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6004: {
		1: {
			ID: 6004,
			Index: 1,
			SkillName: '知己知彼',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {copy:{"num":(LvUp) => {return 2+LvUp;},"pileType":4,"reduceMP":0}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6005: {
		1: {
			ID: 6005,
			Index: 1,
			SkillName: '引经克敌',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":50}},
			ActionCount: 2,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 6005,
			Index: 2,
			SkillName: '引经克敌',
			Target: {"type":"single","team":1},
			Actions: {cleanBuff:{"type":1,"num":1}},
			ActionCount: 2,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6006: {
		1: {
			ID: 6006,
			Index: 1,
			SkillName: '',
			Target: {},
			Actions: {},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6011: {
		1: {
			ID: 6011,
			Index: 1,
			SkillName: '燃袍制敌',
			Target: {"type":"self","team":0},
			Actions: {armBomb:{"delay":10,"dmgRate":0.5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6012: {
		1: {
			ID: 6012,
			Index: 1,
			SkillName: '抛书释卷',
			Target: {"type":"all","team":1},
			Actions: {addBuff:{"buffID":6012,"time":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6013: {
		1: {
			ID: 6013,
			Index: 1,
			SkillName: '以书为御',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":6013,"time":15}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 6013,
			Index: 2,
			SkillName: '以书为御',
			Target: {"type":"self","team":0},
			Actions: {breakArm:{"breakArm":400,"delay":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6014: {
		1: {
			ID: 6014,
			Index: 1,
			SkillName: '借袍于友',
			Target: {"type":"single","team":0},
			Actions: {exchangeArm:{"exchangeArm":1} },
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6015: {
		1: {
			ID: 6015,
			Index: 1,
			SkillName: '带刺玫瑰',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":6015}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	6016: {
		1: {
			ID: 6016,
			Index: 1,
			SkillName: '疼痛滞后',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":6016,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	7001: {
		1: {
			ID: 7001,
			Index: 1,
			SkillName: '一剑封喉',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":800}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 7001,
			Index: 2,
			SkillName: '一剑封喉',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":7101,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	7002: {
		1: {
			ID: 7002,
			Index: 1,
			SkillName: '碎影诀',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":10,"count":30}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	7003: {
		1: {
			ID: 7003,
			Index: 1,
			SkillName: '残光诀',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":8,"count":18}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	7004: {
		1: {
			ID: 7004,
			Index: 1,
			SkillName: '疾风诀',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":6,"count":12}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	7005: {
		1: {
			ID: 7005,
			Index: 1,
			SkillName: '干将莫邪',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":7005}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	7006: {
		1: {
			ID: 7006,
			Index: 1,
			SkillName: '韬光养晦',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 7006,
			Index: 2,
			SkillName: '韬光养晦',
			Target: {"type":"self","team":0},
			Actions: {comboTrigger:{"combo":100,"skillID":7106}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	7106: {
		1: {
			ID: 7106,
			Index: 1,
			SkillName: '韬光养晦',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8001: {
		1: {
			ID: 8001,
			Index: 1,
			SkillName: '丐帮帮主',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":8001}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8101: {
		1: {
			ID: 8101,
			Index: 1,
			SkillName: '丐帮帮主',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":8101,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8002: {
		1: {
			ID: 8002,
			Index: 1,
			SkillName: '急风为刺',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":8002}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8003: {
		1: {
			ID: 8003,
			Index: 1,
			SkillName: '割股充饥',
			Target: {"type":"self","team":0},
			Actions: {damage:{"dmg":200}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 8003,
			Index: 2,
			SkillName: '割股充饥',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		3: {
			ID: 8003,
			Index: 3,
			SkillName: '割股充饥',
			Target: {"type":"self","team":0},
			Actions: {comboTrigger:{"combo":25,"skillID":8103}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8103: {
		1: {
			ID: 8103,
			Index: 1,
			SkillName: '割股充饥',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8004: {
		1: {
			ID: 8004,
			Index: 1,
			SkillName: '借力杀敌',
			Target: {"type":"outsideItself","team":0},
			Actions: {strengthTransition:{"rate":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8005: {
		1: {
			ID: 8005,
			Index: 1,
			SkillName: '天下无狗',
			Target: {"type":"all","team":1},
			Actions: {repetitiousDmg:{"dmg":30,"count":4}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8006: {
		1: {
			ID: 8006,
			Index: 1,
			SkillName: '斗犬七弄',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":50,"count":7}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8011: {
		1: {
			ID: 8011,
			Index: 1,
			SkillName: '守护金兰',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":8011}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8111: {
		1: {
			ID: 8111,
			Index: 1,
			SkillName: '守护金兰',
			Target: {"type":"all","team":1},
			Actions: {damage:{"dmg":80}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 8111,
			Index: 2,
			SkillName: '守护金兰',
			Target: {"type":"all","team":1},
			Actions: {addBuff:{"buffID":8111,"time":0.5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8012: {
		1: {
			ID: 8012,
			Index: 1,
			SkillName: '反伤护甲',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":8012,"time":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8013: {
		1: {
			ID: 8013,
			Index: 1,
			SkillName: '观其动向',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":8013,"time":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8113: {
		1: {
			ID: 8113,
			Index: 1,
			SkillName: '观其动向',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8014: {
		1: {
			ID: 8014,
			Index: 1,
			SkillName: '力挽狂澜',
			Target: {"type":"all","team":1,"singing":0.7},
			Actions: {addBuff:{"buffID":2012,"time":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8015: {
		1: {
			ID: 8015,
			Index: 1,
			SkillName: '鹑衣结发',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":8015,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	8016: {
		1: {
			ID: 8016,
			Index: 1,
			SkillName: '拒死贪生',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":8016,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 8016,
			Index: 2,
			SkillName: '拒死贪生',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":8017,"time":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18001: {
		1: {
			ID: 18001,
			Index: 1,
			SkillName: '木藤刺',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":60}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 18001,
			Index: 2,
			SkillName: '木藤刺',
			Target: {},
			Actions: {spawnSummoned:{"type":"wSword","num":2,"area":"random"}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18003: {
		1: {
			ID: 18003,
			Index: 1,
			SkillName: '木剑雨',
			Target: {"type":"all","team":1,"singing":0.7},
			Actions: {damage:{"dmg":60}},
			ActionCount: 0,
			Animation: 'attack_04',
			CriticalTime: 66,
			Effect: 'rainofsword',
			EffectType: [{"type":"point","origin":"target"}],
			EffectiveTime: 151,
			Path: 'chenjinchou',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [3.5,3.6,3.7,3.8,4.0,4.3,4.5],
			DmgFlag: ""
		},
		2: {
			ID: 18003,
			Index: 2,
			SkillName: '木剑雨',
			Target: {},
			Actions: {spawnSummoned:{"type":"wSword","numType":"hit","area":"random"}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18004: {
		1: {
			ID: 18004,
			Index: 1,
			SkillName: '剑灵乱舞',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {addBuff:{"buffID":18004}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18104: {
		1: {
			ID: 18104,
			Index: 1,
			SkillName: '剑灵乱舞',
			Target: {"type":"random","team":1},
			Actions: {damage:{"dmg":20}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18005: {
		1: {
			ID: 18005,
			Index: 1,
			SkillName: '横剑摆渡',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {addBuff:{"buffID":18005}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18105: {
		1: {
			ID: 18105,
			Index: 1,
			SkillName: '横剑摆渡',
			Target: {"type":"self","team":0},
			Actions: {normalDrawCard:{"num":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18006: {
		1: {
			ID: 18006,
			Index: 1,
			SkillName: '桃木刃匣',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {create:{"cardID":1001,"num":3}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18007: {
		1: {
			ID: 18007,
			Index: 1,
			SkillName: '枯木逢春',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {normalDrawCard:{"num":2}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18008: {
		1: {
			ID: 18008,
			Index: 1,
			SkillName: '取竹成剑',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {normalDrawCard:{"num":1}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 18008,
			Index: 2,
			SkillName: '取竹成剑',
			Target: {"type":"self","team":0},
			Actions: {create:{"cardID":18002,"num":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18009: {
		1: {
			ID: 18009,
			Index: 1,
			SkillName: '秋镰游刃',
			Target: {"type":"all","team":1,"singing":0.7},
			Actions: {reverse:{"type":"wSword","dmg":45,"consume":1}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: '',
			EffectType: [],
			EffectiveTime: 33,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	18010: {
		1: {
			ID: 18010,
			Index: 1,
			SkillName: '映日剑轮',
			Target: {"type":"single","team":1,"singing":1.17},
			Actions: {swordWheel:{"type":"wSword","dmg":25,"consume":0}},
			ActionCount: 0,
			Animation: 'attack_03',
			CriticalTime: 66,
			Effect: 'swordwheel',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 484,
			Path: 'chenjinchou',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [4.4,4.5,4.6,4.7,4.9,5.1,5.3],
			DmgFlag: 1
		}
	},
	90001: {
		1: {
			ID: 90001,
			Index: 1,
			SkillName: '90001',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":90001}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90501: {
		1: {
			ID: 90501,
			Index: 1,
			SkillName: '90501',
			Target: {"type":"random","team":1},
			Actions: {damage:{"dmg":20}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90002: {
		1: {
			ID: 90002,
			Index: 1,
			SkillName: '90002',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":90002}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90502: {
		1: {
			ID: 90502,
			Index: 1,
			SkillName: '90502',
			Target: {"type":"all","team":1},
			Actions: {damage:{"dmg":50}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90003: {
		1: {
			ID: 90003,
			Index: 1,
			SkillName: '90003',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {repetitiousDmg:{"dmg":60,"count":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90004: {
		1: {
			ID: 90004,
			Index: 1,
			SkillName: '90004',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":55}},
			ActionCount: 4,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90004,
			Index: 2,
			SkillName: '90004',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":90004}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90005: {
		1: {
			ID: 90005,
			Index: 1,
			SkillName: '90005',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {repetitiousDmg:{"dmg":55,"count":8}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90006: {
		1: {
			ID: 90006,
			Index: 1,
			SkillName: '90006',
			Target: {"type":"all","team":1,"singing":0.7},
			Actions: {repetitiousDmg:{"dmg":50,"count":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90007: {
		1: {
			ID: 90007,
			Index: 1,
			SkillName: '90007',
			Target: {"type":"all","team":1,"singing":0.7},
			Actions: {repetitiousDmg:{"dmg":44,"count":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90008: {
		1: {
			ID: 90008,
			Index: 1,
			SkillName: '暴雨梨花',
			Target: {"type":"random","team":1,"singing":0.7},
			Actions: {repetitiousDmg:{"dmg":10,"count":27}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90009: {
		1: {
			ID: 90009,
			Index: 1,
			SkillName: '90009',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {randomCreate:{"MPConsume":6,"consumeType":1,"cardType":1,"rate":1,"num":2,"pileType":4}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90010: {
		1: {
			ID: 90010,
			Index: 1,
			SkillName: '90010',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":700}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90011: {
		1: {
			ID: 90011,
			Index: 1,
			SkillName: '90011',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":500}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90011,
			Index: 2,
			SkillName: '90011',
			Target: {"type":"single","team":1},
			Actions: {HPTrigger:{"HP":1500,"HPType":3,"skillID":90511}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90511: {
		1: {
			ID: 90511,
			Index: 1,
			SkillName: '90511',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":500}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90012: {
		1: {
			ID: 90012,
			Index: 1,
			SkillName: '90012',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":900}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90012,
			Index: 2,
			SkillName: '90012',
			Target: {"type":"single","team":1},
			Actions: {dmgTrigger:{"dmg":1200,"dmgType":1,"skillID":90512}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90512: {
		1: {
			ID: 90512,
			Index: 1,
			SkillName: '90512',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":90512,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90013: {
		1: {
			ID: 90013,
			Index: 1,
			SkillName: '90013',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {create:{"cardID":90513,"rate":1,"num":1,"pileType":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90513: {
		1: {
			ID: 90513,
			Index: 1,
			SkillName: '90513',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":90513}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90014: {
		1: {
			ID: 90014,
			Index: 1,
			SkillName: '90014',
			Target: {"type":"random","team":1,"singing":0.7},
			Actions: {create:{"cardID":90514,"rate":1,"num":1,"pileType":1}},
			ActionCount: 4,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90514: {
		1: {
			ID: 90514,
			Index: 1,
			SkillName: '90514',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":90514}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90015: {
		1: {
			ID: 90015,
			Index: 1,
			SkillName: '90015',
			Target: {"type":"single","team":0,"singing":0.7},
			Actions: {addBuff:{"buffID":90015,"time":15}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90016: {
		1: {
			ID: 90016,
			Index: 1,
			SkillName: '90016',
			Target: {"type":"single","team":0,"singing":0.7},
			Actions: {addBuff:{"buffID":90016,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90017: {
		1: {
			ID: 90017,
			Index: 1,
			SkillName: '90017',
			Target: {"type":"single","team":0,"singing":0.7},
			Actions: {addBuff:{"buffID":90017,"time":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90018: {
		1: {
			ID: 90018,
			Index: 1,
			SkillName: '90018',
			Target: {"type":"single","team":0,"singing":0.7},
			Actions: {addBuff:{"buffID":90018,"time":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90018,
			Index: 2,
			SkillName: '90018',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":90518,"time":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90019: {
		1: {
			ID: 90019,
			Index: 1,
			SkillName: '90019',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":90019}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90020: {
		1: {
			ID: 90020,
			Index: 1,
			SkillName: '90020',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":90020}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90520: {
		1: {
			ID: 90520,
			Index: 1,
			SkillName: '90520',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":90520}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90021: {
		1: {
			ID: 90021,
			Index: 1,
			SkillName: '90021',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":90021}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90521: {
		1: {
			ID: 90521,
			Index: 1,
			SkillName: '90521',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":90521,"time":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90022: {
		1: {
			ID: 90022,
			Index: 1,
			SkillName: '90022',
			Target: {type:"self","team":0},
			Actions: {addBuff:{"buffID":90022}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90522: {
		2: {
			ID: 90522,
			Index: 2,
			SkillName: '90522',
			Target: {type:"self","team":0},
			Actions: {addBuff:{"buffID":90522,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90023: {
		1: {
			ID: 90023,
			Index: 1,
			SkillName: '90023',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":100}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90023,
			Index: 2,
			SkillName: '90023',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":90023,"time":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90024: {
		1: {
			ID: 90024,
			Index: 1,
			SkillName: '90024',
			Target: {type:"self","team":0},
			Actions: {reduceMP:{"MPCondition":10,"reduceMP":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90025: {
		1: {
			ID: 90025,
			Index: 1,
			SkillName: '90025',
			Target: {type:"self","team":0},
			Actions: {consumeAndUseCard:{"num":2,"cardType":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90026: {
		1: {
			ID: 90026,
			Index: 1,
			SkillName: '90026',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {normalDrawCard:{"num":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90026,
			Index: 2,
			SkillName: '90026',
			Target: {type:"self","team":0},
			Actions: {heal:{"heal":100}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90027: {
		1: {
			ID: 90027,
			Index: 1,
			SkillName: '90027',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {normalDrawCard:{"num":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90028: {
		1: {
			ID: 90028,
			Index: 1,
			SkillName: '90028',
			Target: {"type":"self","team":0,"singing":0.7},
			Actions: {drawCard:{"num":1,"cardType":1,"piletype":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90028,
			Index: 2,
			SkillName: '90028',
			Target: {type:"self","team":0},
			Actions: {drawCard:{"num":1,"cardType":1,"piletype":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90029: {
		1: {
			ID: 90029,
			Index: 1,
			SkillName: '90029',
			Target: {type:"self","team":0},
			Actions: {dropCard:{"num":1,"piletype":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90029,
			Index: 2,
			SkillName: '90029',
			Target: {type:"self","team":0},
			Actions: {getMP:{"MP":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90030: {
		1: {
			ID: 90030,
			Index: 1,
			SkillName: '90030',
			Target: {type:"self","team":0},
			Actions: {cleanBuff:{"type":1,"num":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90030,
			Index: 2,
			SkillName: '90030',
			Target: {type:"self","team":0},
			Actions: {getMP:{"MP":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	90031: {
		1: {
			ID: 90031,
			Index: 1,
			SkillName: '90031',
			Target: {type:"self","team":0},
			Actions: {damage:{"dmg":200}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 90031,
			Index: 2,
			SkillName: '90031',
			Target: {type:"self","team":0},
			Actions: {getMP:{"MP":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91001: {
		1: {
			ID: 91001,
			Index: 1,
			SkillName: '冰刃锥',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {damage:{"dmg":(LvUp) => {return 90+LvUp;}}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'ice_shoot',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 151,
			Path: 'chenjinchou',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 91001,
			Index: 2,
			SkillName: '冰刃锥',
			Target: {"type":"self","team":0},
			Actions: {normalDrawCard:{"num":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		3: {
			ID: 91001,
			Index: 3,
			SkillName: '冰刃锥',
			Target: {},
			Actions: {spawnSummoned:{"type":"wSword","num":(LvUp) => {return 1+LvUp;},"area":"random"}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91002: {
		1: {
			ID: 91002,
			Index: 1,
			SkillName: '冰石乱坠',
			Target: {"type":"single","team":1,"singing":0.7},
			Actions: {bounce:{dmg:(LvUp) => {return 70+LvUp;},"buffID":1001,"count":(LvUp) => {return 1+LvUp;},"bounce":(LvUp) => {return 2+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'ricochet_ice,ws_ricochet_ice',
			EffectType: [{"type":"point","origin":"owner","offset":{"x":100,"y":300}},{"event":"onFinish","beforeName":"ricochet_ice","type":"bounce","origin":"owner","offset":{"x":100,"y":300}}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91003: {
		1: {
			ID: 91003,
			Index: 1,
			SkillName: '符灵术',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {normalDrawCard:{"num":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91004: {
		2: {
			ID: 91004,
			Index: 2,
			SkillName: '符灵术',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":(LvUp) => {return 1+LvUp;}}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91005: {
		1: {
			ID: 91005,
			Index: 1,
			SkillName: '冰灵劲',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":1008}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: 'abi',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 91005,
			Index: 2,
			SkillName: '冰灵劲',
			Target: {},
			Actions: {costSummoned:{"costSummoned":(LvUp) => {return 5+LvUp;},"type":"wSword"}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91006: {
		1: {
			ID: 91006,
			Index: 1,
			SkillName: '女娲体质',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":4005}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91106: {
		1: {
			ID: 91106,
			Index: 1,
			SkillName: '女娲体质',
			Target: {"type":"self","team":0},
			Actions: {getMP:{"MP":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'healfog3,healflash',
			EffectType: [{"type":"bullt","origin":"owner"},{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'snow',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91007: {
		1: {
			ID: 91007,
			Index: 1,
			SkillName: '释放灵力',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {normalDrawCard:{"num":2}},
			ActionCount: 0,
			Animation: 'skill_04',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 91007,
			Index: 2,
			SkillName: '释放灵力',
			Target: {"type":"self","team":0},
			Actions: {damage:{"dmg":120}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91008: {
		1: {
			ID: 91008,
			Index: 1,
			SkillName: '涛澜动地',
			Target: {"type":"single","team":1,"singing":1.3},
			Actions: {damage:{"dmg":150}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'groundfire',
			EffectType: [{"type":"bullt","origin":"target"}],
			EffectiveTime: 66,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 91008,
			Index: 2,
			SkillName: '涛澜动地',
			Target: {"type":"self","team":0},
			Actions: {damage:{"dmg":30}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91009: {
		1: {
			ID: 91009,
			Index: 1,
			SkillName: '石破天惊',
			Target: {"type":"single","team":1,"singing":1.3},
			Actions: {damage:{"dmg":300}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'aerolite,healfog',
			EffectType: [{"type":"point","origin":"target"},{"type":"point","origin":"owner"}],
			EffectiveTime: 333,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 91009,
			Index: 2,
			SkillName: '石破天惊',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":4010,"time":8}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91010: {
		1: {
			ID: 91010,
			Index: 1,
			SkillName: '火凤落霞',
			Target: {"type":"single","team":1,"singing":1.3},
			Actions: {bounce:{dmg:20,"buffID":4011,"count":1,"time":15,"bounce":4}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 66,
			Effect: 'aerolite,healfog',
			EffectType: [{"type":"point","origin":"target"},{"type":"point","origin":"owner"}],
			EffectiveTime: 333,
			Path: 'snow',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91011: {
		1: {
			ID: 91011,
			Index: 1,
			SkillName: '火焰外衣',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {addBuff:{"buffID":4014}},
			ActionCount: 0,
			Animation: 'talent_01',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		1: {
			ID: 91011,
			Index: 1,
			SkillName: '火焰外衣',
			Target: {"type":"random","team":1},
			Actions: {damage:{"dmg":20}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91012: {
		1: {
			ID: 91012,
			Index: 1,
			SkillName: '火中取栗',
			Target: {"type":"self","team":0,"singing":1.3},
			Actions: {normalDrawCard:{"num":2}},
			ActionCount: 0,
			Animation: 'skill_04',
			CriticalTime: 66,
			Effect: 'ghostcard',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	91013: {
		1: {
			ID: 91013,
			Index: 1,
			SkillName: '朱雀展翅',
			Target: {"type":"all","team":1,"singing":1.3},
			Actions: {addBuff:{"buffID":4011,"time":15}},
			ActionCount: 3,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100001: {
		1: {
			ID: 100001,
			Index: 1,
			SkillName: '蛟精普攻',
			Target: {"type":"single","team":1,"singing":0.4},
			Actions: {damage:{"dmg":90}},
			ActionCount: 0,
			Animation: 'attack_01',
			CriticalTime: 66,
			Effect: 'atk_ice',
			EffectType: [{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: 'hit_ice',
			HitEffectPath: 'hit_ice',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100002: {
		1: {
			ID: 100002,
			Index: 1,
			SkillName: '蛟精猛击',
			Target: {"type":"single","team":1,"singing":1},
			Actions: {damage:{"dmg":250}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 66,
			Effect: 'skill_01',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: 'hit_ice',
			HitEffectPath: 'hit_ice',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 100002,
			Index: 2,
			SkillName: '蛟精猛击',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":100002,"time":8}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100003: {
		1: {
			ID: 100003,
			Index: 1,
			SkillName: '蛟精巨浪',
			Target: {"type":"all","team":1,"singing":0.83},
			Actions: {damage:{"dmg":300}},
			ActionCount: 0,
			Animation: 'skill_02',
			CriticalTime: 66,
			Effect: 'billow,mouth_splash',
			EffectType: [{"type":"point","origin":"targetCenter"},{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: 'hit_ice',
			HitEffectPath: 'hit_ice',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 100003,
			Index: 2,
			SkillName: '蛟精巨浪',
			Target: {"type":"all","team":1},
			Actions: {addBuff:{"buffID":100003,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100004: {
		1: {
			ID: 100004,
			Index: 1,
			SkillName: '蛟精狂风',
			Target: {"type":"single","team":1,"singing":0.2},
			Actions: {damage:{"dmg":360}},
			ActionCount: 0,
			Animation: 'skill_03',
			CriticalTime: 66,
			Effect: 'wind',
			EffectType: [{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 100004,
			Index: 2,
			SkillName: '蛟精狂风',
			Target: {"type":"single","team":1},
			Actions: {dropCard:{"num":2,"cardType":1,"cardQuality":1,"cardAttributes":1,"piletype":2}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100005: {
		1: {
			ID: 100005,
			Index: 1,
			SkillName: '蛟精幻象',
			Target: {"type":"self","team":0,"singing":1.1},
			Actions: {monsterSummoned:{"monsterID":"100002","num":1,"matrixType":"random","time":12}},
			ActionCount: 0,
			Animation: 'skill_04',
			CriticalTime: 0,
			Effect: 'phantom',
			EffectType: [{"type":"point","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'shark',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 100005,
			Index: 2,
			SkillName: '蛟精幻象',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":100105,"time":12}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100101: {
		1: {
			ID: 100101,
			Index: 1,
			SkillName: '鳄鱼普攻',
			Target: {"type":"single","team":1,"singing":1},
			Actions: {damage:{"dmg":60}},
			ActionCount: 0,
			Animation: 'attack_01',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100102: {
		1: {
			ID: 100102,
			Index: 1,
			SkillName: '鳄鱼撕咬',
			Target: {"type":"single","team":1,"singing":1},
			Actions: {damage:{"dmg":90}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 0,
			Effect: 'bitehitbleed',
			EffectType: [{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'crocodile_effect',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 100102,
			Index: 2,
			SkillName: '鳄鱼撕咬',
			Target: {"type":"single","team":1},
			Actions: {addBuff:{"buffID":100102}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100201: {
		1: {
			ID: 100201,
			Index: 1,
			SkillName: '飞颅攻击',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":180}},
			ActionCount: 0,
			Animation: 'attack_01',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100202: {
		1: {
			ID: 100202,
			Index: 1,
			SkillName: '飞颅地震',
			Target: {"type":"all","team":1},
			Actions: {damage:{"dmg":150}},
			ActionCount: 0,
			Animation: 'attack_02',
			CriticalTime: 0,
			Effect: 'trailingshakeDust',
			EffectType: [{"type":"point","origin":"target"}],
			EffectiveTime: 0,
			Path: 'head_effect',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100301: {
		1: {
			ID: 100301,
			Index: 1,
			SkillName: '怨鬼攻击',
			Target: {"type":"single","team":1,"singing":1},
			Actions: {damage:{"dmg":150}},
			ActionCount: 0,
			Animation: 'attack_01',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100302: {
		1: {
			ID: 100302,
			Index: 1,
			SkillName: '怨鬼诅咒',
			Target: {"type":"single","team":1,"singing":1},
			Actions: {damage:{"dmg":50}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 0,
			Effect: 'curse',
			EffectType: [{"type":"bullt","origin":"owner"}],
			EffectiveTime: 0,
			Path: 'spectre_effect',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		},
		2: {
			ID: 100302,
			Index: 2,
			SkillName: '怨鬼诅咒',
			Target: {"type":"single","team":1},
			Actions: {create:{"cardID":100303,"num":1}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	100303: {
		1: {
			ID: 100303,
			Index: 1,
			SkillName: '怨鬼诅咒',
			Target: {"type":"self","team":0},
			Actions: {damage:{"dmg":300}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101101: {
		1: {
			ID: 101101,
			Index: 1,
			SkillName: '攻击',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":100}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'gold_ingot',
			EffectType: [{"type":"bullt","origin":"owner","delay":0.9}],
			EffectiveTime: 0,
			Path: 'jinbanhua_effect',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101102: {
		1: {
			ID: 101102,
			Index: 1,
			SkillName: '判官三指',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":50}},
			ActionCount: 3,
			Animation: 'attack_01',
			CriticalTime: 0,
			Effect: 'wsricochet_ice',
			EffectType: [{"type":"bullt","origin":"owner","delay":1.7}],
			EffectiveTime: 151,
			Path: 'chenjinchou',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101201: {
		1: {
			ID: 101201,
			Index: 1,
			SkillName: '攻击',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":100}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'gold_ingot',
			EffectType: [{"type":"bullt","origin":"owner","delay":0.9}],
			EffectiveTime: 0,
			Path: 'jinbanhua_effect',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101202: {
		1: {
			ID: 101202,
			Index: 1,
			SkillName: '地裂波',
			Target: {"type":"all","team":1},
			Actions: {damage:{"dmg":220}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101203: {
		1: {
			ID: 101203,
			Index: 1,
			SkillName: '金刚护体',
			Target: {"type":"self","team":0},
			Actions: {addBuff:{"buffID":101203,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101301: {
		1: {
			ID: 101301,
			Index: 1,
			SkillName: '攻击',
			Target: {"type":"single","team":1,"singing":0.57},
			Actions: {damage:{"dmg":100}},
			ActionCount: 0,
			Animation: 'attack_01',
			CriticalTime: 66,
			Effect: 'gold_ingot',
			EffectType: [{"type":"bullt","origin":"owner","delay":0.9}],
			EffectiveTime: 0,
			Path: 'jinbanhua_effect',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101302: {
		1: {
			ID: 101302,
			Index: 1,
			SkillName: '疗伤',
			Target: {"type":"self","team":0,"singing":0.57},
			Actions: {heal:{"healPct":0.3}},
			ActionCount: 0,
			Animation: 'skill_01',
			CriticalTime: 66,
			Effect: 'heal',
			EffectType: [{"type":"point","origin":"owner","delay":1.9}],
			EffectiveTime: 99,
			Path: 'heal',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101303: {
		1: {
			ID: 101303,
			Index: 1,
			SkillName: '砸元宝',
			Target: {"type":"all","team":1,"singing":0.57},
			Actions: {damage:{"dmg":300}},
			ActionCount: 0,
			Animation: 'skill_02',
			CriticalTime: 66,
			Effect: 'ingot_fall,ingot_fall_ground',
			EffectType: [{"type":"point","origin":"targetCenter"},{"type":"point","origin":"targetCenter"}],
			EffectiveTime: 99,
			Path: 'jinbanhua_effect',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101401: {
		1: {
			ID: 101401,
			Index: 1,
			SkillName: '攻击',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":150}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: 'gold_ingot',
			EffectType: [{"type":"bullt","origin":"owner","delay":0.9}],
			EffectiveTime: 0,
			Path: 'jinbanhua_effect',
			HitEffect: 'hiteffect',
			HitEffectPath: 'chenjinchou',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101402: {
		1: {
			ID: 101402,
			Index: 1,
			SkillName: '防御',
			Target: {"type":"single","team":0},
			Actions: {addBuff:{"buffID":101402,"time":5}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101403: {
		1: {
			ID: 101403,
			Index: 1,
			SkillName: '听音辨物',
			Target: {"type":"single","team":0},
			Actions: {},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101404: {
		1: {
			ID: 101404,
			Index: 1,
			SkillName: '鹰撮霆击',
			Target: {"type":"all","team":1},
			Actions: {damage:{"dmg":200}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	101405: {
		1: {
			ID: 101405,
			Index: 1,
			SkillName: '鹰喙',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":300}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	102101: {
		1: {
			ID: 102101,
			Index: 1,
			SkillName: '出剑',
			Target: {"type":"single","team":1},
			Actions: {damage:{"dmg":150}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	102102: {
		1: {
			ID: 102102,
			Index: 1,
			SkillName: '三剑',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":18,"count":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	102103: {
		1: {
			ID: 102103,
			Index: 1,
			SkillName: '十剑',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":18,"count":10}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	102104: {
		1: {
			ID: 102104,
			Index: 1,
			SkillName: '三十六剑',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":18,"count":36}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
	102105: {
		1: {
			ID: 102105,
			Index: 1,
			SkillName: '停剑',
			Target: {"type":"single","team":1},
			Actions: {repetitiousDmg:{"dmg":18,"count":3}},
			ActionCount: 0,
			Animation: '',
			CriticalTime: 0,
			Effect: '',
			EffectType: [],
			EffectiveTime: 0,
			Path: '',
			HitEffect: '',
			HitEffectPath: '',
			HitTime: [],
			DmgFlag: ""
		}
	},
};
